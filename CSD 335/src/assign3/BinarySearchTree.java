 ...
 * @athor Starter Code provider. (David Hartley for toString())
 */
package assign3;
...
public class BinarySearchTree extends BinaryTree
{
    ...
    /** Override method of toString. Will return a string representation of the binary search tree
     * in the format { element element ... n } with the elements in-order.
     *
     * @return The string representation of the Binary Search Tree
     */
    @Override
    public String toString() {
        return "{" + getStringTree(root) + "}";
    }

    /** Private method that will traverse the tree and get all the elements of the BST following an
     * In-order traversal
     *
     * @param t The current node object to traverse through
     * @return All the elements in the BST
     */
    private String getStringTree(Node t) {
        //If we are at the leaf node return
        if(t == null)
            return "";

        //Return the left - current - right nodes
        return getStringTree(t.left) + " " + t.value + " " + getStringTree(t.right);
    }
